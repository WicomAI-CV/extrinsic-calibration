...
# DIRECTORY
# LOAD_CHECKPOINT_DIR = f"checkpoint_weights/{MODEL_NAME}_20240617_180353_best_val.pth.tar"
...
class rgb_mono_projection:
    def __init__(self, image_size, K_int, range_mode=False):
    ...
    def __call__(self, pcd_mis, pcd_pred, pcd_gt, rgb_im):
        rgb_img = (T.ToPILImage())(rgb_im).resize((self.W, self.H), Image.Resampling.BILINEAR)
        ...
        pixel_proj_gt = np.matmul(self.K_int, pcd_cam_gt.T).T
        pixel_proj_mis = np.matmul(self.K_int, pcd_cam_mis.T).T
        ...
        depth_gt = np.array([np.linalg.norm(x) for x in pcd_cam_gt]) if self.range_mode else z_axis_gt
        depth_mis = np.array([np.linalg.norm(x) for x in pcd_cam_mis]) if self.range_mode else z_axis_mis

        condition = (0<=u)*(u<self.W)*(0<=v)*(v<self.H)*(depth>0)*(z_axis>=0)
        condition_gt = (0<=u_gt)*(u_gt<self.W)*(0<=v_gt)*(v_gt<self.H)*(depth_gt>0)*(z_axis_gt>=0)
        condition_mis = (0<=u_mis)*(u_mis<self.W)*(0<=v_mis)*(v_mis<self.H)*(depth_mis>0)*(z_axis_mis>=0)
        # print(np.min(z_axis))
        ...
        fig1.tight_layout()
        fig1.savefig('test_comparison_rescaled.png', transparent=False)
